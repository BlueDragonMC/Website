...
export default function NavItem({ text, href, active, color, activeColor, icon }: NavItemProps) {
    
    const path = usePathname();
    const isActive = active || path === href || (href !== "/" && path?.startsWith(href));
    const activeColorClasses = activeColor ?? "bg-slate-500 font-bold hover:bg-slate-400";
